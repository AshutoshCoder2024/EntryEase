  ...
  useEffect(() => {
    ...
    const handleScan = async (payload: string) => {
      try {
        ...
        // Only accept ticket IDs matching the format generated by the server.
        // This prevents HTML injection via QR payloads and reduces DB load.
        const ticketIdStr = String(ticketId).trim();
        const ticketIdOk = /^EVT-[A-Z0-9_-]{10}$/.test(ticketIdStr);
        if (!ticketIdOk) {
          setScan({
        ...
        if (error || !data) {
          setScan({
            message: "Invalid Ticket",
            variant: "error",
            detailHtml: `No ticket found with ID <span class="font-mono">${escapeHtml(ticketIdStr)}</span>.`,
          });
          return;
        }
        ...
        if (data.payment_status !== "verified") {
          setScan({
            ...
            variant: "warning",
            detailHtml: `Ticket for <span class="font-semibold">${escapeHtml(String(data.name ?? ""))}</span> — payment status: <span class="font-mono">${escapeHtml(String(data.payment_status ?? ""))}</span>.`,
          });
          return;
        }
        ...
        if (data.entry_status === "used") {
          setScan({
            ...
            variant: "warning",
            detailHtml: `Ticket <span class="font-mono">${escapeHtml(String(data.ticket_id ?? ""))}</span> for <span class="font-semibold">${escapeHtml(String(data.name ?? ""))}</span> has already been checked in.`,
          });
          return;
        }
        ...
        setScan({
          ...
          variant: "success",
          detailHtml: `Welcome, <span class="font-semibold">${escapeHtml(String(data.name ?? ""))}</span>!<br/>Ticket: <span class="font-mono">${escapeHtml(String(data.ticket_id ?? ""))}</span>`,
        });
      } catch (err) {
        console.error(err);
        setScan({
                  ...
                  <table className="w-full min-w-[800px] text-left text-sm">
                    <thead>
                      <tr className="border-b border-white/10 bg-slate-800/50">
                        ...
                        <th className="px-4 py-3.5 font-semibold text-slate-400">Filled</th>
                        <th className="px-4 py-3.5 font-semibold text-slate-400">Verified</th>
                    ...
                    <tbody>
                      {filtered.map((r) => (
                        <tr key={r.id} className="border-b border-white/5 transition hover:bg-white/5">
                          ...
                          <td className="px-4 py-3.5 text-slate-400">{formatTimestamp(r.created_at)}</td>
                          <td className="px-4 py-3.5 text-slate-400">{formatTimestamp(r.verified_at)}</td>
                          <td className="px-4 py-3.5 font-mono text-xs text-slate-500">
                            {r.ticket_id ?? "—"}
                          </td>
                          <td className="px-4 py-3.5">
